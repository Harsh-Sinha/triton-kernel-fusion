...
def fused_matmul_comm_kernel(
    ...
    # Cast accumulator to float32 before storing
    acc_int32 = acc.to(tl.float32)
...
if __name__ == "__main__":
    ...
    # Allocate input and output tensors on the GPU using float32
    A = torch.randint(-128, 127, (M, K), dtype=torch.float32, device='cuda')
    B = torch.randint(-128, 127, (K, N), dtype=torch.float32, device='cuda')
    C = torch.zeros((M, N), dtype=torch.float32, device='cuda')  # Accumulator needs to be float32
    ...
    # Verification
    # Convert float32 tensors to float32 for PyTorch matmul and then convert the result back to float32
